...
/* Numerical Recipes standard error handler */
{
	...
	/*Arquivo de erro impresso para o GiD em caso de năo alocaçăo dinâmica*/
	char fileerr[1024];
	...
	fprintf(ferr, "\n \n \n *****  ERRO: Memória Insuficiente ou qualquer outro problema de alocaçăo ***** \n");
...
/* allocate an unsigned int vector of zeros with subscript range v[nl..nh] */
{
	unsigned int *v;
	v = (unsigned int *)calloc((nh - nl + 1 + NR_END), sizeof(unsigned));
...
{
	unsigned int *v;
	v = (unsigned int *)malloc((size_t)((nh - nl + 1 + NR_END) * sizeof(unsigned)));
	if (!v) nrerror("allocation failure in uivector()");
	return v - nl + NR_END;
}
...
double **dmatrix_zeros(long nrl, long nrh, long ncl, long nch)
/* allocate a double matrix of zeros with subscript range m[nrl..nrh][ncl..nch] */
{
	long i, nrow = nrh - nrl + 1, ncol = nch - ncl + 1;
	double **m;
	/* allocate pointers to rows */
